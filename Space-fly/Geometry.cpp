...
QPointF Geometry::get_left_down(Rocket rocket) {
    ...
    yc = rocket.yr + 4*rocket.hr/5 - rocket.hr/2;
    xc = xc2 - sqrt(pow(r,2) - pow(yc - yc2,2));
    xc -= sin(M_PI*60/180) * (rocket.hr/3);
    yc += cos(M_PI*60/180) * (rocket.hr/3);
    yc += rocket.hr/2;
...
int Geometry::getLeftAngle(int x, int y, int width, int height) {
    if (y < 0) {
        return atan(x/(qFabs(y) + height/2));
    } else {
        return atan(x/(height/2 - y));
    }
}

int Geometry::getRightAngle(int x, int y, int width, int height) {
    if (y < 0) {
        return atan((width - x)/(qFabs(y) + height/2));
    } else {
        return atan((width - x)/(height/2 - y));
    }
}
